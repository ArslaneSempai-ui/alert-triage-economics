...
export const PLAUSIBLE: Record<keyof Assumptions, [number, number]> = {
...
export type Band = {
  ...
  stableFrom: number;
  stableTo: number;
  /** What the recommendation is at the value in use. */
  recommends: number | null;
  /**
   * What actually changes when you step outside the band.
   *
   * "This assumption decides the recommendation" is not actionable on its own — a
   * recommendation is one of ten discrete thresholds and moves at the first opportunity.
   * What a reader needs is the consequence: one more productive hour per analyst per day
   * moves coverage from 81 % to 91 % at no extra cost, and that is a sentence someone can
   * act on.
   */
  consequence: { at: number; threshold: number; coverage: number } | null;
  decides: boolean;
  /**
   * Why it does not decide, when it does not.
   *
   * "Priced out" and "insensitive" look identical on a dashboard and mean opposite
   * things. One says the number is irrelevant; the other says the binding constraint is
   * somewhere else and this number would matter the moment that changed.
   */
  reason: "decides" | "another constraint binds" | "genuinely insensitive";
};

const pop = generatePopulation();
const answerOf = (a: Assumptions) => recommend(pop, undefined, a)?.threshold ?? null;

export function band(assumption: keyof Assumptions, a = ASSUMPTIONS, steps = 40): Band {
  const [low, high] = PLAUSIBLE[assumption];
  const reference = answerOf(a);
  const current = a[assumption];

  const walk = (toward: number): number => {
  ...
  let consequence: Band["consequence"] = null;
  if (decides) {
    ...
    if (r) consequence = { at: outside, threshold: r.threshold, coverage: r.coverageAfter };
  }
...
export function bands(a = ASSUMPTIONS): Band[] {
  return (Object.keys(PLAUSIBLE) as (keyof Assumptions)[]).map((k) => band(k, a));
}

/** What the screen tells whoever just typed a number in. */
export function advise(b: Band): string {
  const f = (x: number) => (x < 100 ? x.toFixed(1) : Math.round(x).toLocaleString("en-GB"));
  if (b.reason === "decides") {
