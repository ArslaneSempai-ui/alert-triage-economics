...
export type Resource = "analyst" | "productiveHour" | "handlingDay";

export const RESOURCES: Record<Resource, {
  label: string;
  /** What one unit costs per year. `null` where the resource is not bought with money. */
  unitCost: (a: Assumptions) => number | null;
  apply: (a: Assumptions, units: number) => Assumptions;
  /** How far to search for the next step. Wide enough to find it, not a recommendation. */
  maxUnits: number;
  /** Granularity of the search. Analysts come in ones; hours do not. */
  step: number;
...
/**
 * One step of the staircase — not one unit of the resource.
 *
 * Listing every unit prints a table that is mostly the word "nothing", and a reader skims
 * past the one line that matters. Only the values at which the recommendation actually
 * moves are kept, each carrying how many units of dead ground it sits behind.
 */
export type Rung = {
  units: number;
  threshold: number;
  /** Reportable cases found at this rung. */
  truePositives: number;
  coverage: number;
  annualCost: number;
  /** Cases gained by this step, against the rung below it. */
  gained: number;
  /** Units bought since the previous rung — the width of the step, and its whole price. */
  width: number;
  /** What the whole step cost per case it gained. `null` where the resource is not money. */
  perTruePositive: number | null;
};

/**
 * Why buying more stops working.
 *
 * Naming the binding constraint is the whole value of the exercise. A team told "more
 * analysts no longer help" spends the budget somewhere useful; a team told nothing keeps
 * asking for heads.
 */
export type Blocker =
  | "the monitoring system, not the team"
  | "the queue diverges before the money runs out"
  | "the handling deadline"
  | "nothing; it still buys";

export type ShadowPrice = {
  resource: Resource;
  unitCost: number | null;
  /** Only the values at which the recommendation moves, starting from the current one. */
  rungs: Rung[];
  /** The widest run of units that buys nothing — the trap a partial budget falls into. */
  widestDeadZone: number;
  blocker: Blocker;
};
 ...
 */
function blockerAt(pop: Population, a: Assumptions): Blocker {
  const r = recommend(pop, THRESHOLDS, a);
  if (!r) return "the queue diverges before the money runs out";

  const i = THRESHOLDS.indexOf(r.threshold);
  if (i === THRESHOLDS.length - 1) return "the monitoring system, not the team";

  const next = evaluate(pop, THRESHOLDS[i + 1]!, a);
  if (!next.queueHolds) return "the queue diverges before the money runs out";
  if (!next.deadlineMet) return "the handling deadline";
...
export function shadowPrices(pop = generatePopulation(), a = ASSUMPTIONS): ShadowPrice[] {
  return (Object.keys(RESOURCES) as Resource[]).map((r) => shadowPrice(r, pop, a));
}

/** The one sentence a committee needs. */
export function verdict(s: ShadowPrice): string {
  const spec = RESOURCES[s.resource];
  const money = (x: number) => "$" + Math.round(x).toLocaleString("en-GB");
  const unit = (n: number) => quantity(s.resource, n);
 ...
 */
export function quantity(resource: Resource, n: number): string {
  ...
  if (resource === "handlingDay") return `${n} more day${n === 1 ? "" : "s"} of handling time`;
 ...
 *
 * This is the finding, and no single resource's table contains it. The first step down —
 * from a 0.50 threshold to 0.45, 43 more reportable cases a year — is reachable by hiring
 * an analyst for $62,000, by extending the handling target from five working days to six,
 * or by finding fifteen more productive minutes a day. Same step, same cases, and two of
 * the three routes cost no money at all.
 *
 * A committee that only ever sees the headcount paper approves the headcount. The tool's
 * job is to put the free route on the same page as the priced one.
 */
export type SameStep = {
  threshold: number;
  gained: number;
  /**
   * The numbers, and an English rendering for the console and the README.
   *
   * `units` is what the bilingual screen needs: a sentence built on the server exists in
   * exactly one language, and shipping "1 more day of handling time" into a French page is
   * an architecture defect rather than a missing translation. The same mistake was made
   * once in the triage tool and fixed there for the same reason.
   */
  routes: { resource: Resource; units: number; amount: string; cost: number | null }[];
  /**
   * What the free route costs in something other than money.
   *
   * Extending the handling target is free on a budget line and not free at all: it is
   * spent out of the margin between the queue and the 30-day wall at
   * `31 CFR 1020.320(b)(3)`. Printing "free" beside it without this is the kind of
   * half-true a finance paper gets right and a compliance paper does not.
   *
   * The two figures are in different units and the first version subtracted one from the
   * other. The queue model works in **working** days; the regulation counts **calendar**
   * days, and says so — "within 30 calendar days after the date of the initial
   * detection". 5.5 working days is 9.1 calendar days, and the margin was overstated by
   * three and a half days by an arithmetic that looked fine.
   */
  deadlineCost: {
    waitWorkingDays: number;
    waitCalendarDays: number;
    wallCalendarDays: number;
    marginCalendarDays: number;
  } | null;
};

export function cheapestRouteToNextStep(pop = generatePopulation(), a = ASSUMPTIONS): SameStep | null {
  const prices = shadowPrices(pop, a);
  const firsts = prices
    .map((p) => ({ p, r: p.rungs[1] }))
    .filter((x): x is { p: ShadowPrice; r: Rung } => x.r !== undefined);
  ...
  let deadlineCost: SameStep["deadlineCost"] = null;
  if (viaDeadline) {
...
if (isMain(import.meta)) {
  ...
  const unit = (n: number) => (n % 1 === 0 ? String(n) : n.toFixed(2));
  ...
  for (const s of shadowPrices(pop)) {
    ...
    for (const r of s.rungs) {
      ...
      console.log(
        `  ${unit(r.units).padStart(6)}   ${(r.width === 0 ? "n/a" : "+" + unit(r.width)).padStart(4)}` +
  ...
  console.log(
    "A slope through this table would report one average price per case and hide the only" +
