...
export const leadQuarters = (h) => Math.ceil(h.hiringLeadWeeks / QUARTER_WEEKS);
/**
 ...
 */
export function headsRequired(operations, h, a, ceiling = 400) {
...
function cumulativeLoss(q, perQuarter, from) {
 ...
 */
export function decisionUnderGrowth(rates = [0.02, 0.04, 0.06, 0.08, 0.10, 0.15], h = HORIZON, a = ASSUMPTIONS) {
    return rates.map((g) => {
...
if (isMain(import.meta)) {
    ...
    if (p.hires.length === 0) {
        ...
        for (const x of p.hires) {
            console.log(`  ${q(x.decideIn).padEnd(14)}${q(x.arriveIn).padEnd(11)}` +
    ...
    if (step) {
        ...
        console.log(`  Going from ${step.from.toFixed(2)} to ${step.to.toFixed(2)} costs no money this quarter: ${via}.` +
            (step.freeUntil === null
    ...
    for (const r of decisionUnderGrowth()) {
        console.log(`  ${((r.growth * 100).toFixed(0) + " %").padStart(10)}` +
